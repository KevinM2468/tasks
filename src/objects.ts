...
export function isValid(question: Question, answer: string): boolean {
    ...
    // The documentation does not include any more examples
    // I will assume this means there are no other valid cases
    return false;
}
...
export function toShortForm(question: Question): string {
    ...
    work += question.id;
    work += ": ";
    work += question.name.substring(0, 10);
    return work;
}
...
export function addOption(question: Question, newOption: string): Question {
    const ops = [...question.options, newOption];
    return { ...question, options: ops };
}
